...
async function printMonth(
	...
	const monthName = monthNumber
		? MONTH_NAMES_DISPLAY[monthNumber]
		: monthFolderName;
...
async function printMonthEntries(jerdPath, monthNumber, year) {
	...
	if (!(await fileExists(monthPath)) || !(await isDirectory(monthPath))) {
		errorMessage(
			`No entries found for ${MONTH_NAMES_DISPLAY[monthNumber]} ${year}`,
		);
		return;
	}
	...
	if (journalFiles.length === 0) {
		infoMessage(
			`No journal entries in ${MONTH_NAMES_DISPLAY[monthNumber]} ${year}`,
		);
