...
export function renderMonthlyChart(moodData, title) {
	...
	const sortedMoods = Object.entries(moodCounts).sort(
		(a, b) => MOOD_VALUES[b[0]].value - MOOD_VALUES[a[0]].value,
	);
...
function renderStats(moodData) {
	...
	if (entriesWithMood.length > 0) {
		const avgMood =
			entriesWithMood.reduce((sum, e) => sum + e.value, 0) /
	...
	if (entriesWithoutMood.length > 0) {
		console.log(
			dimText(`${entriesWithoutMood.length} entries had no mood data`),
		);
	}

	if (missingEntries.length > 0) {
		console.log(
			dimText(`${missingEntries.length} days without journal entries`),
		);
	}
}
