...
function renderStreakStats(activityData) {
  ...
  const percentage =
    totalDays > 0 ? ((totalEntries / totalDays) * 100).toFixed(0) : 0;
  ...
  // Stats in a row
  console.log(
    chalk.hex("#39d353")(`🔥 ${currentStreak}`) +
      dimText(" current") +
      "  " +
      chalk.hex("#f59e0b")(`⭐ ${longestStreak}`) +
      dimText(" longest") +
      "  " +
      chalk.hex("#3b82f6")(`📝 ${totalEntries}`) +
      dimText(` entries (${percentage}%)`),
  );
