...
export function stepLine(step, total, text) {
	...
	console.log(
		`${C().muted(currentTheme.icons.treeVertical)} ${stepText} ${C().text(
			text,
		)}`,
	);
}
...
export async function landingScreen(context = {}) {
	...
	const actions = initialized
		? [
				...
				{
					cmd: 'jerd open yesterday',
					desc: "continue yesterday's thoughts",
					icon: '📖',
				},
	...
	const actionLines = actions.map(
		a =>
			`${a.icon} ${C().info(a.cmd.padEnd(maxCmd + 2))}${C().muted(
				`- ${a.desc}`,
			)}`,
	);

	const footer = `${C().muted('Run')} ${C().info('jerd --help')} ${C().muted(
	...
	const tipLine = initialized
		? C().muted('💡 Tip: Personalize your experience with ') +
		  C().info('jerd config --theme')
		: C().muted('💡 Tip: Start your journaling journey with ') +
		  C().info('jerd init .');
...
function getStatsDisplay(stats) {
	...
	if (stats.entriesThisMonth > 0) {
		parts.push(
			`${C().info('📝 ' + stats.entriesThisMonth + ' entries this month')}`,
		);
	}
...
export function notInitializedBanner() {
	const content = [
		`${C().warning(currentTheme.icons.warning)}  ${chalk
			.hex('#ef4444')
			.bold('Not a jerd project')}`,
		'',
		`${C().muted('Run')} ${C().info('jerd init')} ${C().muted(
			'to get started',
		)}`,
	].join('\n');
